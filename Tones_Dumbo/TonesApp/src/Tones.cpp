...
void Tones::setup(){
    ...
    brightThreshold= admin.gui.getValueF("CAM1:BRIGHT");
    darkThreshold= admin.gui.getValueF("CAM1:DARK");
...
void Tones::update(){
    ...
    if (camera.update()){ //returns true if frame is new
        ...
        unsigned char * current=grayImage.getPixels();
        unsigned char * bg=grayBg.getPixels();
        ofPixels diff;
        diff.allocate(grayImage.getWidth(), grayImage.getHeight(), OF_IMAGE_GRAYSCALE);
        
        for(int i=0;i<grayImage.getWidth()*grayImage.getHeight();i++){
            if (current[i]-bg[i]>brightThreshold) diff[i]=255;
            else if(bg[i]-current[i]>darkThreshold) diff[i]=255;
            else diff[i]=0;
        }
        ofPixels diffSet;
        grayDiff.setFromPixels(diff);
        grayDiff.blur(7);
...
//        grayDiff.threshold(threshold);
        ...
        contourFinder.findContours(grayDiff, 10, (camWidth*camHeight)/3, 10, false);	// find holes
    }
...
//        if(bNewBlob==true){
            ...
            ofVec2f blobToCenter = blobClosest-ofVec2f(camWidth/2,camHeight/2);

        tempBlob.closest=ofVec2f(camWidth/2,camHeight/2)+blobToCenter*camShift;
    ...
    if(mouseAsBlob){
        ...
        for (int i=0; i<nMouseBlobs; i++){
            ...
            if(i == 0){
                ...
                mouse.x = mouseX-100;
                mouse.y = ofGetHeight() - mouseY-100;
            } else break;
            pts.push_back(mouse);
        }
    }
