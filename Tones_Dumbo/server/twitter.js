...
mongo.open(function(err,mongo){
	...
	dbTones = mongo.db('dumbo_arts_fest_fri');
...
var twitter = ImmortalNTwitter.create(EZ1);




var trackPhrases = ['#tones' ];
...
oscServer.on("error",function(msg, rinfo){
...
var getAudioFile= function(callback){
	...
	colTweets.find(obj).sort(sort).limit(1).toArray( function(err,res){
		...
		if(res.length< 1){
			...
			//var new_obj = {timestamp : {$gt: last_timestamp}}
			 var new_sort = { last_played:1 };
			 colTweets.find(new_obj).sort(new_sort).limit(1).toArray( function(err,res){
					if(err) console.error(err);

					if(res.length<1){
						var newest_sort = {last_played: 1 };
						colTweets.find().sort(newest_sort).limit(1).toArray(function(err,res){
							if(err) console.error(err);
							last_timestamp = res[0].timestamp;
							callback(res[0]);
						});
					}else{
						last_timestamp = res[0].timestamp;
